...
When the document is a dydo skill, read [`skill-mechanics`](.agents/skills/writing-for-agents/resources/skill-mechanics.md) first: frontmatter keys, the invocation choice, Must-Reads, resources, and what `dydo sync` compiles. When the text is the reply only the human will read, `writing-for-humans` is this skill's pair.
