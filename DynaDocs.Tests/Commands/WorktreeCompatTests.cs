...
public class WorktreeCompatTests : IDisposable
{
    ...
    #region PathPermissionChecker — worktree path resolution

    [Fact]
    public void PathPermissionChecker_WorktreeBasePath_ResolvesToMainRoot()
    {
    ...
    [Fact]
    public void PathPermissionChecker_WorktreeBasePath_BlocksUnauthorizedPath()
    {
        var mainRoot = Path.Combine(_testDir, "main-repo");
        var worktreeBase = Path.Combine(mainRoot, "dydo", "_system", ".local", "worktrees", "fix-auth");
        Directory.CreateDirectory(worktreeBase);
        ...
        var agent = new AgentState
        {
            Name = "Alice",
            Role = "code-writer",
            WritablePaths = ["Commands/**"],
            ReadOnlyPaths = ["**"]
        };

        var absPath = Path.Combine(mainRoot, "dydo", "config.json");
        Assert.False(checker.IsPathAllowed(agent, absPath, "write", out var error));
        Assert.Contains("cannot write", error);
    }

    [Fact]
    public void PathPermissionChecker_RelativePath_WorksInWorktreeContext()
    {
        var mainRoot = Path.Combine(_testDir, "main-repo");
        var worktreeBase = Path.Combine(mainRoot, "dydo", "_system", ".local", "worktrees", "fix-auth");
        Directory.CreateDirectory(worktreeBase);
        ...
        Assert.True(checker.IsPathAllowed(agent, "Services/MyService.cs", "write", out _));
    }
