...
public class WatchdogServiceTests : IDisposable
{
    ...
    public void PollAndResumeForAgent_LiveClaimedPid_DoesNotLaunch()
    {
        ...
        var launchCount = 0;
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, _, _, _) => { launchCount++; return 0; };
    ...
    public void PollAndResumeForAgent_DeadClaimedPid_BelowCap_Launches()
    {
        ...
        WatchdogService.LaunchResumeOverride = (a, s, _, _, _, _, _, _, _) => { calls.Add((a, s)); return 12345; };
    ...
    [Fact]
    public void PollAndResumeForAgent_CodexSession_PassesCodexHostToLauncher()
    {
        var dydoRoot = ResumeDydoRoot();
        WriteResumeAgentState(dydoRoot, "Adele", status: "working", resumeAttempts: 0);
        WriteResumeAgentSession(dydoRoot, "Adele", "sess-codex", 99999999, host: "codex");
        ProcessUtils.IsProcessRunningOverride = _ => false;
        WatchdogService.ResumeAttemptsCapOverride = 3;

        string? capturedHost = null;
        WatchdogService.LaunchResumeOverride = (_, _, h, _, _, _, _, _, _) => { capturedHost = h; return 12345; };

        WatchdogService.PollAndResumeCrashedAgents(dydoRoot);

        Assert.Equal("codex", capturedHost);
        Assert.Equal(1, ReadResumeAttempts(dydoRoot, "Adele"));
    }

    [Fact]
    public void PollAndResumeForAgent_PassesStateRoleToLauncher()
    {
        var dydoRoot = ResumeDydoRoot();
        WriteResumeAgentState(dydoRoot, "Adele", status: "working", resumeAttempts: 0, role: "code-writer");
        WriteResumeAgentSession(dydoRoot, "Adele", "sess-codex", 99999999, host: "codex");
        ProcessUtils.IsProcessRunningOverride = _ => false;
        WatchdogService.ResumeAttemptsCapOverride = 3;

        string? capturedRole = null;
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, _, _, role) => { capturedRole = role; return 12345; };

        WatchdogService.PollAndResumeCrashedAgents(dydoRoot);

        Assert.Equal("code-writer", capturedRole);
        Assert.Equal(1, ReadResumeAttempts(dydoRoot, "Adele"));
    }
    ...
    public void PollAndResumeForAgent_RepeatedPolls_StopAtCap()
    {
        ...
        var launchCount = 0;
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, _, _, _) =>
        {
            launchCount++;
    ...
    public void PollAndResumeForAgent_LaunchesWithProjectRootAsWorkingDirectory()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, wd, _, _, _, _, _) => { capturedWorkingDirectory = wd; return 12345; };
    ...
    public void PollAndResumeForAgent_PersistedWindowId_PassedToLauncher()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, _, w, t, _, _, _) =>
        {
    ...
    public void PollAndResumeForAgent_WithWorktreeMarker_PassesWorktreeContextToLaunchResume()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, wt, mr, _) =>
        {
    ...
    public void PollAndResumeForAgent_NoWorktreeMarker_PassesNullWorktreeId()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, wt, _, _) =>
        {
            capturedWorktreeId = wt;
            return 12345;
        };
    ...
    public void PollAndResumeForAgent_LaunchSucceeds_PersistsLaunchedPidToState()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, _, _, _) => 54321;
    ...
    public void PollAndResumeForAgent_LaunchFailed_LeavesLaunchedPidNull()
    {
        ...
        WatchdogService.LaunchResumeOverride = (_, _, _, _, _, _, _, _, _) => 0;
        ...
        string host = "claude")
    {
        ...
        Directory.CreateDirectory(agentDir);
        var pidJson = claimedPid.HasValue ? claimedPid.Value.ToString() : "null";
        var json = $"{{\"Agent\":\"{agentName}\",\"SessionId\":\"{sessionId}\",\"Host\":\"{host}\",\"Claimed\":\"{DateTime.UtcNow:o}\",\"ClaimedPid\":{pidJson}}}";
        File.WriteAllText(Path.Combine(agentDir, ".session"), json);
    }
