...
def _coverage_module(opencover, root, assembly, aliases):
...
def _document_origins(assembly):
    ...
    return documents, document_origins


def _coverage_files(module, documents, document_origins):
    files = {}
...
def _eligible_coverage_methods(assembly, excluded_tokens):
    ...
    def requires_coverage(row):
        return any(point.get("origin") == "maintained" for point in row.get("points", []))
...
def _sequence_rows(method, token, files, expected):
    file_rows = {}
    ...
    return file_rows, point_lines


def _branch_rows(method, token, files, point_lines, file_rows, branch_uspids):
    branch_ids = set()
...
def _coverage_method(method, methods, files, seen_tokens, branch_uspids):
    ...
    file_rows, point_lines = _sequence_rows(method, token, files, expected)
    _branch_rows(method, token, files, point_lines, file_rows, branch_uspids)
    return identity, {"token": token, "files": file_rows}
...
def coverage_methods(opencover, root, assembly, aliases, excluded_tokens=frozenset()):
    ...
    module = _coverage_module(opencover, root, assembly, aliases)
    documents, origins = _document_origins(assembly)
    files = _coverage_files(module, documents, origins)
    methods = _eligible_coverage_methods(assembly, excluded_tokens)
    result, seen_tokens, branch_uspids = {}, set(), set()
    for method in module.findall("./Classes/Class/Methods/Method"):
        row = _coverage_method(method, methods, files, seen_tokens, branch_uspids)
        if row is not None:
            identity, facts = row
...
def _origin_accounting(physical, generated_files, maintained_files):
    all_points = physical["points"]
    if not all_points:
        return {"identity": physical["identity"], "reason": "no non-hidden portable-PDB points"}
    ...
    if "maintained" not in origins:
        ...
        return {"identity": physical["identity"], "reason": "excluded by origin",
                "origins": sorted(origins), "documents": sorted(documents)}
    missing = sorted(documents - maintained_files)
    if missing:
        raise ValueError(f"Maintained PDB document absent from source inventory: {missing[0]}")
    return None


def _logical_owner(physical, constructors, fragments, source_methods, declared):
    key, points = physical["key"], physical["points"]
    if key not in constructors:
        path, owner = _normal_owner(physical, source_methods, declared)
        return path, owner, (path, owner["id"]), None
    ...
    accounting = {"identity": physical["identity"],
                  "constructor_order": _constructor_order(physical, constructor, fragments)}
    return path, owner, None, accounting


def _validate_authored_methods(source_methods, mapped):
...
def join_methods(root, source, assembly, coverage):
    ...
    for physical in assembly["methods"]:
        origin_row = _origin_accounting(physical, generated_files, set(modules))
        if origin_row is not None:
            accounting.append(origin_row)
            continue
        points = list(physical["points"])
        if not points:
        ...
        path, owner, mapped_key, owner_accounting = _logical_owner(
            physical, constructors, fragments, source_methods, declared)
        if mapped_key is not None:
            mapped.add(mapped_key)
        if owner_accounting is not None:
            accounting.append(owner_accounting)
    ...
    _validate_authored_methods(source_methods, mapped)
