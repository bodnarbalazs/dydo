...
public static class HubGenerator
{
    ...
    // Hardcoded prose appended to the project hub. Task files are transient
    // (one per agent role-assignment) and not auto-indexed (D4). Updating
        ...
        List<DocFile> allDocs)
    {
        ...
        if (PathUtils.NormalizePath(relativeFolderPath)
    ...
    public static Dictionary<string, string> GenerateAllHubs(string basePath, List<DocFile> allDocs)
    {
        ...
        // D4: project/tasks is no longer auto-indexed — task files are transient.
        foldersToProcess.RemoveWhere(f =>
            f.Equals("project/tasks", StringComparison.OrdinalIgnoreCase));
        ...
        Dictionary<string, string> generatedHubs)
    {
        ...
        var existingHubs = allDocs.Where(d =>
        {
            ...
            // D4: project/tasks/_index.md is no longer surfaced as a subfolder hub.
            if (docDir.Equals("project/tasks", StringComparison.OrdinalIgnoreCase))
                return false;
