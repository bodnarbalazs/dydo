...
How to deepen a cluster of shallow modules safely, given its dependencies. Assumes the vocabulary in the
`codebase-design` skill: **module**, **interface**, **seam**, **adapter**.
...
Dependencies that have local test stand-ins (PGLite for Postgres, in-memory filesystem). Deepenable if
...
in-memory adapter for testing, so the logic sits in one deep module even though it's deployed across a
network."*
...
- Tests should survive internal refactors, since they describe behaviour, not implementation. If a test
