...
A tick is **O(changes), not O(corpus)** — a doc base 100× this repo syncs just as comfortably. Each tick asks Notion only for pages edited on or after its stamp cursor (one server-side filtered query per type) and stat-walks the repo for changed files, then reconciles only that changed set; a body is fetched only for a genuinely-new or just-edited page, so a **steady quiet tick costs exactly one filtered query per type — zero body reads** (the watchdog log records `requests` per tick). Provisioning/schema validation runs on a cadence (~every 20 ticks), on start, and after an error — not every tick. Because a filtered query cannot see archived pages, remote deletions are caught by a periodic **census** (`--census-interval <ticks>`, default 240 ≈ hourly) — a body-free id/stamp pagination whose disappeared ids surface archives, fuse-guarded. The full everything-reconcile stays with the manual `dydo notion sync`, run rarely and on purpose (which also seeds the daemon's cheap-tick state, so a watchdog started afterwards begins warm). `dydo watchdog run` is the foreground loop itself, used by `start`.
