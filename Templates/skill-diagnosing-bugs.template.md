...
When exploring the codebase, read [`glossary.md`](../../../glossary.md) and
[`architecture.md`](../../../understand/architecture.md) to get a clear mental model of the relevant
...
`<REDACTED>` in its place. Build loops against env vars, so the credential stays in the environment
...
**This is the skill.** Everything else is mechanical. If you have a **tight** pass/fail signal for the
bug (one that goes red on _this_ bug), you will find the cause; bisection, hypothesis-testing, and
instrumentation all just consume it. If you don't have one, no amount of staring at code will save you.
...
9. **Differential loop.** Run the same input through old-version vs new-version (or two configs) and
   diff outputs.
10. **HITL script.** Last resort. If a human must click, drive _them_ with a numbered script of steps
    and paste-back points so the loop is still structured. Captured output feeds back to you.
...
add stress, narrow timing windows, inject sleeps. A 50%-flake bug is debuggable; 1% is not, so keep
...
Phase 1 is done when the loop is **tight** and **red-capable**: you can name **one command** (a script
path, a test invocation, a curl) that you have **already run at least once** (show the invocation and
its output, redacted), and that is:

- [ ] **Red-capable**: it drives the actual bug code path and asserts the **Issue's exact symptom**,
...
- [ ] **Agent-runnable**: you can run it unattended; a human in the loop only via a HITL script.
...
- [ ] The loop produces the failure mode the **Issue** described, not a different failure that
...
config, data, and steps **one at a time**, re-running the loop after each cut, and keep only what's
...
If the only available seam is too shallow (single-caller test when the bug needs multiple callers,
unit test that can't replicate the chain that triggered the bug), a regression test there gives false
confidence.
