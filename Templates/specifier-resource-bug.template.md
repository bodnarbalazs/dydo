...
1. Name expected and observed behavior at the highest stable seam. A boundary-visible defect becomes
   a scenario; an internal defect becomes a gate requiring the implementer's red regression test
   through `diagnosing-bugs`. Done when one observation can refute the claimed reproduction.
2. Map reproduce-or-identify, then fix. A trivial known cause may stay on one record; an elusive one
   names hypotheses and proof tests. Reproduction hardens nothing; the fix takes the full hardener
   hop. Done when each step has its evidence and the fix cannot claim success without the red case.
