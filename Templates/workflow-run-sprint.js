export const meta = {
  name: 'run-sprint',
  description: 'Implement a sprint: for each disjoint slice, loop code-writer → reviewer until the review passes (escalating after 5 failed rounds or whenever a worker raises its hand), sequentially merge passed worktree slices back into the invoking branch, then run a final merged-sprint audit (reviewer × merge-sprint) over the entire merged sprint diff.',
...
// A model-bound QA-gate stage (reviewer) returns null from agent() when its
...
if (results.some(r => r.merged)) {
  ...
  auditFindings = audit
    ? [audit.findings, audit.raiseHand && audit.reason ? `Raised hand: ${audit.reason}` : audit.reason]
        .filter(Boolean).join('\n\n')
    : 'the merged-sprint audit did not return a result'
} else {
