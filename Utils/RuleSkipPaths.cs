...
public static class RuleSkipPaths
{
    ...
    /// <summary>
    /// True for paths under <c>_system/template-additions/</c>. Input must be a forward-slash
    /// normalized relative path (use <see cref="PathUtils.NormalizePath"/>).
    /// </summary>
    public static bool IsTemplateAddition(string normalizedRelativePath)
    {
        if (string.IsNullOrEmpty(normalizedRelativePath))
            return false;

        return normalizedRelativePath.StartsWith(TemplateAdditionsPrefix, StringComparison.OrdinalIgnoreCase);
    }
