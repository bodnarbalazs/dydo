...
- Result: PASSED
- Notes: Surgical, test-only fix. InitGitRepo: -c key=val on the commit invocation is the right scope (no global mutation, helper signature unchanged); divergence from WorktreeMergeSafetyIntegrationTests' git-config form is acceptable. ProcessUtilsCaptureTests: /c/Users/User/bin is correct POSIX (longest :* suffix removed -> first PATH entry remains as a non-empty prefix), preserves the test's intent; comment explains the dash-vs-bash WHY. run_tests.py 4131/4131 pass; gap_check.py 140/140 modules pass (exit 0). dydo check has 56 errors but all pre-existing on parent 49c0759 and tracked by existing issues ([#0159](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0159-frontmatter-validtypes-allowed-types-list-missing-inquisition-every-inquisition.md) inquisition type, [#0166](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0166-hub-format-drift-hubgenerator-uses-doc-title-but-committed-index-md-files-use-ke.md) hub/index drift, etc.) - none introduced by this commit.
