...
- [024 — dydo 2.0: Pivot to a Policy + Context Layer over Native Orchestration](./024-dydo-2-native-pivot.md) - dydo drops its worker-tier runtime and becomes the policy (guard) and context (structured docs) layer over Claude Code's native subagents, skills, and workflows. Supersedes 009.
- [025 — Notion Sync: Canonical Files + a Swappable View Adapter](./025-notion-sync-architecture.md) - Repo files stay canonical; Notion is one swappable view adapter behind a Notion-agnostic sync engine that reconciles bidirectionally via base-snapshot + 3-way merge. Sync the data, not the presentation.
