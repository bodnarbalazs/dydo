...
<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0141-wait-guard-deadlock-dydo-wait-auto-exits-on-already-unread-inbox-state-guard-the.md -->
<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md -->
<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md -->
...
| Brian 4c2838f8 | 18:03:06 | 18:04:11 (no_refresh) | **No sidecar at all** — initial JSON only (1 Read event) |
  ...
  - The downstream effect of `SaturateResumeAttempts` on a falsely-blocked session is partially self-healing: once the resumed claude DOES finally make its first tool call, `ClaimAgent → HandleExistingSession → ResetResumeBookkeeping` (`AgentRegistry.cs:347`, [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md) fix) zeroes the cap again. So the cap saturation is not the durable harm. The durable harm is the **false alarm in the watchdog log itself**, which is exactly what the user reads as "auto-resume is broken."
...
- **Issue:** #[0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md --> (covers Findings #1 and #5 — same defect, two angles)

#### 2. Worktree-claimed agents register their watchdog anchor in the wrong directory

- **Category:** bug / cross-cutting infrastructure
- **Severity:** medium-high (contributes to the "watchdog dies prematurely" symptom in [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md))
...
- **Evidence:**
  ...
  - The watchdog's exit conditions in `Run()` (`WatchdogService.cs:325-326`):
    ...
    - When all main-resident anchors die (e.g. orchestrator releases) but worktree-only leaf agents are still working, the watchdog exits via `anchor_gone` — even though leaf agents need it. This matches the open [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) symptom verbatim.
...
- **Proposed fix path:** `AgentRegistry.cs:417` should use `PathUtils.FindMainDydoRoot()` (or equivalent main-resolving helper) for the RegisterAnchor write, mirroring `EnsureRunning`. The two callsites should ideally route through a single helper so the rule is enforced by construction. Cross-references existing [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) (Linux/Mac variant of the same family).
- **Judge ruling:** CONFIRMED
...
- **Alternative explanations considered:** Was the worktree-anchor dir intentionally local — perhaps a per-worktree watchdog was contemplated? No — `WatchdogService.EnsureRunning()` (line 101-105) explicitly resolves `mainDydoRoot = PathUtils.FindMainDydoRoot()` and the comment at line 95-99 says "Resolves to the MAIN project root — dispatches from inside a worktree do not spawn a second, worktree-scoped watchdog." So the design is one-watchdog-per-main, and the `AgentRegistry.cs:417` callsite simply missed that contract. Was the claim-time anchor path added intentionally only for non-worktree dispatches? No — the comment at lines 411-414 says "Closes [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) — without this, leaf agents whose dispatcher has already exited lose watchdog coverage and silently never resume." That intent demands the anchor land in main.
- **Issue:** [#0174](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0174-worktree-claimed-agent-registers-watchdog-anchor-in-worktree-dydo-root-main-watc.md)
...
- **Issue:** [#0175](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0175-auto-resume-launch-path-skips-worktree-wrapper-junctions-init-settings-finally-c.md)
...
- **Evidence:** see Finding #1 for the data; this finding isolates the algorithmic issue.
  ...
  - `SaturateResumeAttempts(cap=3)` is a heavy hammer for a guess. Combined with `ResetResumeBookkeeping` running on the eventual same-session re-claim ([#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md) fix), the saturation does eventually self-clear — but for the duration of the rehydration the agent is "blocked" from a watchdog perspective and would not be resumed if it crashed AGAIN during rehydration.
...
- **Alternative explanations considered:** Could `SaturateResumeAttempts` be adequate as-is because `ResetResumeBookkeeping` ([#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md)) clears the cap on the eventual same-session re-claim? Partly — the cap saturation does self-clear. But the durable harm is twofold: (a) the watchdog log records `resume_blocked` while the agent is in fact recovering, which is what the user reads as "auto-resume is broken"; (b) for the duration of the rehydration, if the claude crashes AGAIN, the watchdog will not resume it (cap is at the limit). Both harms argue for adding the liveness check rather than relying on self-healing.
- **Issue:** Subsumed under [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) (same defect; the issue's "Resolution" section enumerates the liveness-check fix this finding recommends).
...
**Finding #2** (worktree anchor-dir mismatch) is a real bug independent of Finding #1, and contributes to the same family as the open [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) (watchdog dies via `anchor_gone` while leaf agents are still working). It is not the dominant user-visible symptom today but warrants a concrete fix.
...
3. **File a new issue for Finding #2** (anchor-dir mismatch in worktree-claimed agents). It cross-references the existing open [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) but is the Windows variant the existing issue text doesn't cover.
4. **Augment [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)** with the `claude.exe.old.<ts>` post-update process-name case (Finding #3). It's the same fix family.
...
- **Issue:** #[0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md --><!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md -->
...
- **Issue:** subsumed by #[0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md) (Finding #1 install closes both at once).
...
- **Remaining deeper issue #1 (open-ended Bash poll-loops):** CONFIRMED. The 4090052a sidecar tail directly demonstrates the pattern; 4c2838f8's no-sidecar shape is consistent with the same crash class. Filed as [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md). Recommendation: coding-standards rule + custom guard nudge.
- **Remaining deeper issue #2 (same-task self-review gap):** CONFIRMED. Filed as [#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md).
...
- **RECOMMENDED ACTIONS list:** all five sound. #1 (install + watchdog restart) is the gating action — confirmed. #2 (re-run inquisition 24-48h post-install) is sound — PR3's instrumentation makes the 4-bucket count trivial. #3 (coding-standards rule + nudge against open-ended Bash polls) is filed as [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md). #4 (same-task self-review hygiene fix) is filed as [#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md). #5 (defer all source-side fixes to PR1/PR2/PR3) is correct — no fourth source PR needed; the gap is deployment, not authoring.
...
**Verdict summary:** 2 findings CONFIRMED ([#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md) critical deployment gap, subsumes Finding #2); 2 deeper issues CONFIRMED and filed ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) medium poll-loop crash class, [#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md) medium self-review gap); 1 deeper issue FALSE POSITIVE (watchdog log silence — direct verification shows healthy 10s ticks).
...
- Whether the LC project's separate watchdog has the same `claude.exe.old.<ts>` rename condition as DynaDocs's `anchor_name`. LC's last `start` event lists `anchor_name: "claude"` (not the renamed form), which suggests LC's claude has not been updated as recently — so [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)'s specific trigger may be DynaDocs-only at this moment.
...
- **Defensive constraint:** identical to Brian/Dexter prior sections — no full-file Read of any audit JSON or `.events`; sampling via `head` / `tail` / `wc -l` / `grep`. No open-ended Bash poll-loops (`until [ … ]` / `while ! …`) — that is the unrelated crash mode flagged in [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md).
- **Versions in scope:** dydo source-tree HEAD `5bcbe0f` (post-PR3, plus PR3 docs commit). Per Dexter's earlier Finding #1 ([#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)), the running on-disk binary is still pre-PR1; this investigation is conducted against the source code as-of-now and the watchdog log produced by the pre-fix running binary. Where the kill-heuristic question depends on which code is on the user's machine, both are addressed below.
...
- **Alternative explanations considered:**
  ...
  5. *"Could the pre-fix binary (the one actually running on balazs's machine, per [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)) have a different kill path that the post-PR3 source no longer has?"* — Verified by `git log -p -- Services/WatchdogService.cs Commands/WorktreeCommand.cs` between v1.4.0 (a06db96, 2026-04-30) and HEAD: no `Process.Kill` call has been added or removed since v1.4.0; the auto-close gate's `isFree` predicate has not been weakened (if anything it has been hardened by [#0121](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0121-watchdog-kills-re-dispatched-agents-in-poll-gap-stale-decision-clearautoclose-rm.md) fixes); and `KillClaudeProcesses`'s name-whitelist has only been tightened (claude/node), never broadened to "anything matching the substring." Pre-fix behaviour is the same with respect to active-agent kills.
...
- **Issue:** [#0179](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0179-executecleanup-prints-worktree-x-cleaned-up-even-when-removezombiedirectory-fail.md)
...
- **Brian's earlier "Brian f9936e33" / "Charlie 8b52b181" / "Frank 04d1191f" / etc. were NOT crashes** — they all reached `Release` (the prior section's table at lines 421-422 lists these as bucket-(a) successes). The narrative of "many crashes today" overstates the crash count; most of those entries are auto-resume successes that the watchdog log mislabelled as `resume_blocked` (Brian's #1 / [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)).
...
- **Evidence:** the brief asks whether the suspect heuristic exists pre-fix, post-fix, or both. Verified by reading both the running-binary's behaviour fingerprint and the source diff:
  ...
  - **Running-binary side (pre-PR1, per [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)):** the kill-side code in `Services/WatchdogService.cs:KillClaudeProcesses` and the auto-close gate in `PollAndCleanupForAgent` have not changed since v1.4.0 (verified by `git log -p` over those regions for the v1.4.0..HEAD range). The pre-fix binary and the post-PR3 source share the same kill paths and the same `isFree` gate.
...
- **Conclusion:** pre-fix and post-fix both lack a kill-heuristic that escalates from "cannot remove directory" to terminating the holding process. The deployment gap ([#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)) does not change the answer to this brief's question.
...
n/a — verdict is No. For completeness: if a kill-heuristic ever did need to be added in the future, the safe pattern is the auto-close gate already used by `PollAndCleanupForAgent` (line 410) — never kill a PID whose owning agent is in a non-`free` status, and always whitelist by process name to avoid terminal-emulator collateral damage (closes [#0122](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0122-watchdog-kills-linux-terminal-emulator-process-via-argv-leak.md)).
...
- **Crashes attributable to the documented bash-poll-loop pattern ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)):** 1 of 2 (`4090052a`).
...
2. **Install the post-PR3 binary (per Dexter's earlier [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md), still pending).** This remains the single most-impactful action available — it closes the `resume_blocked: no_refresh_after_warmup` log dishonesty, restores PR2's worktree-anchor placement, and turns on PR3's `resume_outcome` instrumentation. The kill-heuristic question does not depend on this install, but the user's broader "auto-resume looks broken" symptom does.
...
4. **For the Charlie-class onboarding crash (Brian's #4 / Dexter's prior #1):** no new fix proposed here. The mitigation is the same as before — keep dispatch briefs as small as possible without losing essential context, and avoid open-ended Bash polls ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)).
...
5 findings reviewed. **5 CONFIRMED, 0 false positives, 0 inconclusive.** Issues filed: **[#0179](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0179-executecleanup-prints-worktree-x-cleaned-up-even-when-removezombiedirectory-fail.md)** (Finding #2, low-severity log-correctness defect). Findings #1, #3, #4, #5 are observations / red-herrings as the inquisitor classified them — no issues warranted. The kill-heuristic verdict (NO) holds: triangulated across source-code walk (every `Process.Kill` site enumerated and gated), production-log forensics (116/116 historical kills carry `status:"free"`, zero exceptions), and Charlie's `fd17b834` audit (1 event, no sidecar, no kill in watchdog log). The misleading `"Worktree X: cleaned up."` log line on Windows file-locks is the contributing factor that made the kill-heuristic hypothesis look plausible to balazs — fixing it ([issue #0179](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0179-executecleanup-prints-worktree-x-cleaned-up-even-when-removezombiedirectory-fail.md)) closes the inference path.
...
| `4090052a` | hung on `until [ -s /tmp/claude/… ]` poll-loop at 15:43 | `resume_blocked` at 15:46 (true crash; [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)) |
...
4. WARNING emitted to stderr; misleading `"cleaned up."` lies on stdout ([issue #0179](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0179-executecleanup-prints-worktree-x-cleaned-up-even-when-removezombiedirectory-fail.md)).
...
- the watchdog's dead-PID detection took 14 minutes to fire — consistent with **[#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)** (Windows watchdog never registers anchors; orphan-cap is the only check), which would substantially delay dead-PID recognition.
...
The issues directory shows **at least 7 distinct, currently-open crash/resume failure modes** filed in the last few weeks: [#0149](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0149-wait-rearm-gap-deadlocks-agent-when-3-plus-messages-stack-faster-than-agent-can-process.md) (wait-rearm flood deadlock), [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) (auto-resume sometimes fails to trigger), [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md) (Windows anchor coverage gap), [#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md) (auto-resume race / duplicate launches), [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md) (resume-attempts not reset on same-session reclaim), [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) (Linux/Mac watchdog dies via anchor-gone), [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) (resume false-positives), [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) (bash poll-loop crash). Today's Charlie deaths are consistent with several of these firing simultaneously, not a single mechanism. The inquisitor's narrow scope ("does a kill-heuristic exist?") was answered correctly but did not address the broader question the user actually wanted answered — *what's killing claude in these silent-death scenarios, and is there a single root cause or several?*
...
3. **How polluted is the watchdog log by [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) false-positive resume_blocked entries?** Sample ratio over the last 30 days — if a meaningful fraction of `resume_blocked` entries are false-positives, every prior crash-pattern analysis grounded in that log is suspect.
4. **Is [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md) (Windows anchor-coverage gap) responsible for the 14-minute dead-PID detection lag?** Direct test would clarify.
...
- **Versions in scope:** dydo source-tree HEAD `5bcbe0f` (post-PR3). On-disk binary still pre-PR1 per [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md) (gating-action: install). Claude Code: locally `2.1.133`; the May-7 incidents ran `2.1.132`.
...
- **Evidence:**
  ...
  - **Session was Charlie working as JUDGE** on Dexter's earlier agent-crashes-followup inquisition (the one that filed [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)/[#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)/[#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md)). Branch `worktree/investigate-agent-crashes-followup`. Claude version `2.1.132`. Model `claude-opus-4-7`.
...
- **Independent verification:** Sampled the JSONL: 256 lines, 582914 bytes (= "583 KB" rounded). Verified `version:"2.1.132"`, `gitBranch:"worktree/investigate-agent-crashes-followup"`, `cwd:"…\\worktrees\\investigate-agent-crashes-followup"`. Last assistant turn confirmed at `2026-05-07T23:10:41.363Z` — a multi-section verdict ruling on Findings #1/#2/deeper-#1/deeper-#2/deeper-#3 of Dexter's prior agent-crashes inquisition, ending with "Awaiting your acknowledgement or follow-up questions" (decisive: this was a JUDGE turn that filed [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)/[#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)/[#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md), not an onboarding death). Final JSONL entry: `system / subtype:"away_summary"` at `2026-05-07T23:13:45.556Z` — proves claude.exe was alive at 23:13:45.
- **Alternative explanations considered:** Could the JSONL be from a different session that shares the same UUID? No — sessionId in every record matches `fd17b834-85dd-47d9-8713-da618d840467`, parentUuid chain is intact, and the away_summary's content names the issues that Charlie filed ([#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md), [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md), [#0178](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0178-same-task-self-review-check-is-task-name-keyed-and-trivially-defeated-by-sub-tas.md)) which are present on disk under the same dates. Could "Brian-class onboarding OOM" still apply on top of this? No — onboarding OOM by definition kills the session before any user-visible turn lands; this session posted a 1248-token assistant verdict.
- **Issue:** none.
...
- **Issue:** none on the gap itself. [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md) remains open as filed; its real-world impact is "watchdog dies after 24 h of orphan-cap when no anchor ever lands" (long-horizon), not "delayed dead-PID detection on a single session" (does not occur).
- **Judge ruling (Frank, 2026-05-08):** CONFIRMED
...
#### Finding #4 — `resume_blocked` events are ~80 % false positives across the full 10-day log retention; [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) magnitude understated

- **Category:** measurement of an existing defect ([#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md))
...
- **Evidence:** Watchdog log span 2026-04-29 17:25 → 2026-05-08 11:08 UTC (~10 days; full retention). Total `resume_blocked` events: **16**, all `reason: no_refresh_after_warmup`.
  ...
  | **4090052a (Charlie)** | 05-07 15:47 | Bash `until [ -s ... ]`; no Release/no later Claim | **TRUE CRASH ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md))** |
...
- **Independent verification:** Per-sid: ran `tail -1 dydo/_system/audit/2026/<sid>*.events` (where the sidecar exists), recording the last event type and timestamp. The two no-sidecar cases (`4c2838f8`, `fd17b834`) are unambiguously NOT clean releases; `4c2838f8` has been previously flagged as a [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)-class no-sidecar crash, and `fd17b834` is documented in Finding #1 above with JSONL transcript evidence. No FP relies on inference — every FP has a concrete `Release` or post-resume_blocked activity record in the sidecar.
- **Issue:** **[#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) is the right home for this** — it already documents the mechanism (60 s gate fires before slow-rehydration recovery completes). The issue body should be updated to reflect the measured magnitude (~80 % FP rate over 10 days; the prior body's qualitative "every resume log line is followed ~60 s later by resume_blocked" is correct but the quantitative impact ought to be explicit). I'm flagging this as "augmentation needed" rather than filing a new issue. **For the judge: please update [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)'s description with the 13/16 = 81 % measurement and link this finding.**
- **Judge ruling (Frank, 2026-05-08):** CONFIRMED
...
- **Independent verification:** Re-enumerated all `resume_blocked` events from the rotated + current watchdog log: count is **16**, all `reason: no_refresh_after_warmup`, all sids/timestamps match the inquisitor's table. Spot-checked 6 of 16 rows against their audit sidecar tails: `5a32806b` ends in `Release` at 21:47:20Z (FP confirmed); `1f852665` ends in `Release` at 18:28:28Z (FP confirmed; agent in events is Charlie, matching the inquisitor's "(Charlie)" label); `4c2838f8` has only the JSON metadata file with no `.events` sidecar (NO_SIDECAR confirmed); `4090052a` ends in `Bash {cmd: "until [ -s /tmp/claude/…/tasks/…"}` at 15:43:23Z with no Release after (TRUE CRASH confirmed, [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) shape verbatim); `1f878107` ends in `Bash` at 17:13:17Z and a clean `Release` at 17:38:10Z, both *post* the `resume_blocked` at 16:32 — so the resume actually worked despite being "blocked," confirming the inquisitor's striking 1-of-13 observation; `fd17b834` is the no-sidecar case proven elsewhere via JSONL. 6 of 16 = 37.5% spot-check coverage; all 6 classifications independently match. Extrapolating to the unsampled 10 rows is justified: the failure mode is structural (the gate fires before the resumed claude's first guard hook lands), so any session with a sidecar Release after `resume_blocked` is mechanically a FP; any no-sidecar / hung-Bash ending is a TC.
...
- **Evidence:**
  - **4090052a (Charlie, 15:42–15:46Z 2026-05-07):** Last sidecar event is `Bash` with cmd `until [ -s /tmp/claude/.../tasks/… ]`. **Direct match to [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)** (open-ended Bash poll-loops crash claude). Already filed; recommended fix is preventative (coding-standards rule + guard nudge).
  - **fd17b834 (Charlie, 23:01–23:16Z 2026-05-07):** No sidecar in main audit (the followup-worktree-local audit was wiped on partial cleanup); **JSONL transcript proves session was a JUDGE turn that idled, then claude.exe silently exited with NO Windows fault event.** Pattern matches upstream `#15001 / #50299 / #55424 / #55562` — the v2.1.114-introduced Windows silent-exit regression family. **Not explainable by any of [#0149](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0149-wait-rearm-gap-deadlocks-agent-when-3-plus-messages-stack-faster-than-agent-can-process.md), [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md), [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md), [#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md), [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md), [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md), [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md), or [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md).**
  - **[#0149](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0149-wait-rearm-gap-deadlocks-agent-when-3-plus-messages-stack-faster-than-agent-can-process.md)** (wait-rearm flood deadlock): symptom is "agent stuck unable to issue tools because every fresh wait exits within ms on the next stacked unread." fd17b834 was a judge with no inbox flood. 4090052a's last action is a bash poll, not a guard-blocked tool call. **No match.**
  - **[#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md)** (auto-resume sometimes fails to trigger): the watchdog DID trigger resume launches on every dead-PID detection in today's data. Resume launches fire correctly. **No match.**
  - **[#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)** (Windows anchor coverage gap): real defect, but per Finding #3 above does NOT explain detection latency for either crash. Long-horizon impact only.
  - **[#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md)** (duplicate launches in warmup gap): no `resume` event for fd17b834 or 4090052a fires more than once per session in today's data. **No match.**
  - **[#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md)** (resume-attempts not reset on same-session reclaim): all today's `resume_blocked` events have `attempts:1`, so cap accumulation is not the today-symptom. May still be a long-running issue. **No match for these specific crashes.**
  - **[#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md)** (Linux/Mac anchor_gone): Windows-only environment. **N/A.**
  - **[#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)** (resume_blocked false positives): explains the 13 false-positive `resume_blocked` events, NOT the underlying claude.exe crashes. **Match for the noise around the crashes, not the crashes themselves.**
- **Conclusion: there is no missing 8th dydo crash mode.** Today's two true crashes are explained by:
  1. one filed dydo defect ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)) for 4090052a, and
  2. one upstream Claude Code defect (no dydo-side fix possible; #15001 / #50299 / #55424 / #55562 family) for fd17b834.
- **Issue:** none new. The recommendation in the closing actions is to (a) update [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) with the 81 % FP measurement and (b) consider adding a dydo-side documentation note about the upstream Windows silent-exit family so future inquisitors don't re-derive this from scratch.
- **Judge ruling (Frank, 2026-05-08):** CONFIRMED
...
- **Independent verification:** Read the symptom-shape of each open issue and independently mapped the two crashes:
  - **4090052a** — last sidecar event is `Bash` with cmd literally `until [ -s /tmp/claude/.../tasks/… ]`, no Release. [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)'s body explicitly cites this as a canonical example ("Charlie 4090052a May 7, sidecar tail is 'until [ -s /tmp/claude/…]'"); recovery rate "collapses to ~0%" for this class because rehydration hits the same crash. Direct, named match. None of [#0149](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0149-wait-rearm-gap-deadlocks-agent-when-3-plus-messages-stack-faster-than-agent-can-process.md)/[#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md)/[#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)/[#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md)/[#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md)/[#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md)/[#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) fit the symptom shape: the watchdog DID detect dead PID and DID fire resume; attempts=1 (not cap-saturated); no inbox flood; environment is Windows; 0173 explains the noise around the crash, not the crash itself.
  - **fd17b834** — JSONL proves a 10-minute judge turn ending with a posted verdict and an idle gap, then silent claude.exe exit; no sidecar Bash entries (so [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) ruled out); no inbox flood (so [#0149](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0149-wait-rearm-gap-deadlocks-agent-when-3-plus-messages-stack-faster-than-agent-can-process.md) ruled out); watchdog detected and resumed (so [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) ruled out); single resume event (so [#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md) ruled out); attempts=1 (so [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md) ruled out); Windows env (so [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md) N/A); [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) explains the resume_blocked noise but not the underlying death; [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md) is real but does not delay detection (Finding #3 verified). The remaining causal slot maps to the upstream silent-exit family — now tracked at **[#0180](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0180-upstream-claude-code-v2-1-114-windows-silent-exit-regression-family-affects-v2-1.md)** (filed by this judge).
- **Alternative explanations considered:** Could there be an unfiled 8th dydo crash mode (e.g. an unhandled exception in a Bash hook for fd17b834)? The JSONL records the last guard-checked Bash at well before 23:10:14Z, with normal completion; the away_summary at 23:13:45Z proves claude.exe was still healthy enough to write JSONL recap entries. No dydo guard call falls in the 23:13:45–23:16:20 dead window. Could the upstream attribution be confounded with [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) if fd17b834's hidden Bash-density was high? Possible but does not make this a *missing 8th mode* — it would still resolve to either the existing [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) (preventative coding-standards rule) or the new external-tracking [#0180](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0180-upstream-claude-code-v2-1-114-windows-silent-exit-regression-family-affects-v2-1.md). Either way, no new dydo-side crash mode is needed.
- **Issue:** none new. The two true crashes resolve cleanly to existing issues + the new external tracker ([#0180](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0180-upstream-claude-code-v2-1-114-windows-silent-exit-regression-family-affects-v2-1.md)); [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) covers the ambient noise.
...
| 4090052a | resume_blocked 15:47 | **TRUE CRASH ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) bash poll-loop)** |
...
So Charlie did NOT crash 4 times today. **Charlie crashed twice today** — once from a known dydo defect ([#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)) and once from an upstream Claude Code bug (no dydo-side fix). The other two `resume_blocked` entries are [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) noise on sessions that completed normally.
...
2. **4090052a poll-loop death** ← [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) (already filed, fix is preventative — coding-standards + guard nudge).
3. **80 % of `resume_blocked` log lines are noise** ← [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) (already filed; magnitude understated in current body).
4. **[#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md) is real but does not explain any of the symptoms balazs has been observing.** It's a long-horizon defect (watchdog dies after 24 h of orphan-cap if no anchor ever registers); it did not delay any dead-PID detection in any of today's data.
...
1. **Stop attributing today's symptoms to the watchdog.** The watchdog's behaviour is correct (per prior section + Findings #1, #3, #5 here). The noise comes from [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)'s false-positive labelling, not from misbehaviour.
2. **Augment [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md)'s body with the 81 % false-positive measurement.** This is the single highest-leverage change to clear the narrative — every analysis grounded in `resume_blocked` counts is currently overstated by ~5×.
3. **Land [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md)'s preventative fix** (coding-standards rule + guard nudge against open-ended bash polls). This closes 1 of 2 today-class crashes.
...
5. **Install the post-PR3 binary ([#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md), still pending).** Restated from prior section. Not a blocker for this inquisition's conclusions, but the longer it stays uninstalled the noisier the watchdog log gets.
...
- **The `~/.claude/projects/` JSONL for 4090052a's late phase** (was the bash poll-loop hang OS-level visible inside the transcript? Probably not, but I didn't sample beyond the last-line check). Low-impact: 4090052a is already attributed to [#0177](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0177-open-ended-bash-poll-loops-until-s-while-tail-f-in-agent-shell-calls-crash-claud.md) with strong evidence.
...
5 findings reviewed. **5 CONFIRMED, 0 false positives, 0 inconclusive.** Issues filed: **[#0180](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0180-upstream-claude-code-v2-1-114-windows-silent-exit-regression-family-affects-v2-1.md)** (Finding #2 — low-severity external tracking issue for the upstream Claude Code v2.1.114+ Windows silent-exit family). [#0173](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0173-auto-resume-resumewarmupgate-60s-produces-false-positive-resume-blocked-no-refre.md) augmented in place with the 13/16 = 81% FP-rate measurement (Finding #4) — magnitude correction, not a new defect. Findings #1, #3, #5 are corrections / red-herrings as the inquisitor classified them — no issues warranted.
...
#### 11. No v1.4.7 commit attempts to address [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md) in code
- **Concern:** G
- **Classification:** clean
- **Severity:** —
...
- **Issue:** [#0181](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md)
...
- **Issue:** [#0182](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0182-fixhubhandler-deletestaletasksindex-banner-gated-deletion-lacks-regression-test.md)
...
#### Finding #11 — No v1.4.7 commit attempts to address [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md)
- **Judge ruling:** CONFIRMED
...
- **Alternative explanations considered:** Could a commit silently address [#0176](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0176-pr1-pr2-pr3-committed-but-not-installed-running-watchdog-binary-predates-all-thr.md) without a grep-match keyword? Inspected the commit titles — all six PR commits are clearly about resume_blocked / worktree anchors / RecoveryClassifier / log truthfulness / nudges / docs. No installer or version-bump artefact.
...
1. Finding #2 → [issue #0181](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md) (pre-existing Saturate-vs-claim race; v1.4.8).
2. Finding #7 → [issue #0182](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0182-fixhubhandler-deletestaletasksindex-banner-gated-deletion-lacks-regression-test.md) (DeleteStaleTasksIndex banner-gated deletion test pin; v1.4.8).
