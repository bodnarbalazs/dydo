...
Investigation of the watchdog auto-resume mechanism (Decision 022) covering two reported defects: [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) (auto-resume sometimes fails to fire after a crash) and the companion [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md --><!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md --><!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md --> (when resume does fire, it always opens a new window instead of reusing the original window's tab group). Both share `Services/WatchdogService.cs` and the per-OS `LaunchResume` paths.

## 2026-05-01 — Brian

### Scope

- **Entry point:** Heavy-gunner inquisition into auto-resume defects [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) and [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md) as a single investigation.
- **Files investigated:**
...
- **Prior fixes reviewed:** [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md --> (`397011f`, RefreshClaimedPid on same-session reclaim), [#0138](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0138-auto-resume-launches-terminal-in-user-home-dir-not-project-root.md) (`473af47`, workingDirectory threaded into resume launch), [#0145](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0145-powershell-routed-dydo-wait-requires-approval-every-re-arm-breaks-unattended-wai.md) (`3808f37`, PowerShell allow-list).
...
- **Evidence:**
  ...
  - `Services/ProcessUtils.Ancestry.cs:60-63` — `MatchesProcessName` is `Path.GetFileNameWithoutExtension(actualName).Equals(needle, OrdinalIgnoreCase)`. **Exact** match on the basename. Comment at `:58-59` explicitly cites [#0128](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0128-findancestorprocess-substring-match-too-broad-claude-vs-claudia.md) to justify exact (not substring) matching.
...
- **Issue:** [#0151](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0151-watchdog-never-registers-anchors-on-windows-orphan-cap-is-the-only-thing-keeping.md)
...
- **Evidence:**
  - `Services/WatchdogService.cs:415-484` — `PollAndResumeForAgent` increments `resume-attempts` and launches the resume terminal, but does **not** mark `.session.ClaimedPid` as alive yet. The resumed claude must run `dydo agent claim` before `RefreshClaimedPid` ([#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md) fix) updates the PID.
  - `Services/AgentRegistry.cs:321-329` — `RefreshClaimedPid` runs only inside the same-sessionId branch of `HandleExistingSession`, which is entered only when `ClaimAgent` is called. Comment from [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md) explicitly notes that, before this fix, *"The next watchdog tick saw a dead PID and fired another resume."*
  ...
  - **Symptom doubling:** during the gap, the user sees 1–3 redundant terminals (the symptom [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md) was filed for, only partially mitigated). Worse — and silently — `resume-attempts` reaches the cap, so **subsequent unrelated crashes never resume** (see also Finding 3, which removes the eventual reset).
- **Repro recipe:**
...
- **Issue:** [#0152](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0152-auto-resume-race-watchdog-fires-duplicate-launches-during-the-resumed-claude-war.md)
...
- **Issue:** [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md)
...
- **Issue:** [#0154](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0154-linux-mac-watchdog-dies-via-anchor-gone-when-all-dispatchers-exit-while-leaf-dis.md)
...
- **Category:** bug (issue [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md))
- **Severity:** low (polish)
- **Type:** obvious (code analysis)
- **Evidence:**
...
- **Alternative explanations considered:** (a) Could the missing thread-through be intentional (e.g. fresh-window-on-resume is a desired isolation property)? No artefact says so — issue [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md) (already filed manually) describes this exact behaviour as a polish defect. (b) Could the Linux side of this finding be subsumed by issue [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md)? Yes, the underlying cause is the same — the resume API simply doesn't accept `useTab`/`windowName`/terminal-name parameters — which is why I'm not filing a new issue and pointing to [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md) instead. (c) Could `wt.exe`'s `--window <new-name>` actually attach to some existing window-by-name? It can — but the GUID is regenerated every resume, so the original window's name is unrecoverable from this path.
- **Issue:** [#0144](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0144-auto-resume-opens-in-new-window-should-reuse-the-original-window-as-a-new-tab-wh.md) (already open; finding confirms the existing issue's root cause without filing a duplicate)
...
For [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) — minimal Windows repro of the dominant root cause (Finding 1):
...
For [#0150](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0150-auto-resume-on-crash-sometimes-fails-to-trigger-watchdog-misses-dead-claude-process.md) — Linux/Mac repro of Finding 4 (anchor_gone):
