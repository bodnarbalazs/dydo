...
the calling process's actual ownership. Triggered by issues [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) (resolved 2026-05-18)
and [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) (open, filed 2026-05-18); commissioned by Adele as a full bug-class
...
- **Files investigated:**
  ...
  - `Services/AgentSelector.cs` (post [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) fix)
...
- Cases where two agents legitimately share state via the worktree junction (`dydo/agents/<A>/state.md` is the same file in main and worktree — this is by design; [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md)'s worktree symptom is hijack-adjacent but the underlying conflict is "two valid claims on the same agent name," not "wrong agent resolved").
...
#### S1. `dydo agent role` write target ([#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) reproduction #1)
...
#### S3. `dydo dispatch` sender identity (round-1 [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) + residual round-2 attribution)
...
- **Failure mode:** Outgoing messages carry the hijacked agent's name as sender. The phantom-inbox file in [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) (`dydo/agents/Zelda/inbox/98e06797-msg-general.md` with `from: Charlie`) was almost certainly produced by this surface — some process with `DYDO_AGENT=Charlie` sent a message to Zelda, the file says From: Charlie because the hijack made GetCurrentAgent return Charlie. Reply-pending markers also clear on the wrong agent.
...
- **Failure mode:** The guard's view of "who is calling" is whichever agent DYDO_AGENT names. Consequences:
  ...
  - NOTICE is sourced from the hijacked agent's UnreadMessages. The phantom-inbox NOTICE in [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) stems from this: Zelda's UnreadMessages has the phantom id but the guard resolves to Charlie (no phantom there), so the NOTICE doesn't fire from the hijacked side; conversely a Zelda-actually-claimed process sees the NOTICE about Zelda's inbox even though it can't write to Zelda's record to clear it.
...
- **What it does:** When claim arrives and an existing session exists for the agent:
  ...
  - **Path B** (same-session reclaim, `existingSession.SessionId == sessionId`): `RefreshClaimedPid` ([#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md)) + `ResetResumeBookkeeping` ([#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md)) + audit event with `recovery_kind=auto` + proceed (idempotent — workspace not regenerated).
...
- **Severity:** **LOW.** The reclaim paths are correctly gated against the hijack via `.pending-session` — credit to the prior [#0103](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0103-no-reclaim-path-for-working-agents-whose-claude-process-is-dead-requires-destruc.md)/[#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md)/[#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md) work. The audit's `recovery_kind=auto` and the resume-outcome event in the watchdog log (architecture.md `Watchdog` section) accurately reflect same-session reclaims; identity isn't muddied here.
...
- **Failure mode:** Two worktree processes for the same agent name race on `dydo/agents/<X>/state.md`, `.session`, and `.session-agent`. This was the round-1 [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) surface (worktree reviewer auto-dispatch onto the orchestrator). Round-1's fix (filter sender from selector pool) closes the *dispatch* surface but the shared-junction *race* is intrinsic — two worktrees running as the same agent name can still produce write-write conflicts on .session.
...
- **Severity:** **MEDIUM.** Known surface; documented in [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md). Not the round-2 root cause but compounds it.
...
- **File:** `Services/WatchdogService.cs` (not re-read in this inquisition, but referenced via [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md), [#0153](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0153-resume-attempts-is-not-reset-on-same-session-reclaims-so-the-counter-accumulates.md), decision 022, architecture.md `Watchdog` section).
...
- **Hijack interaction:** If the watchdog mis-identifies an agent and resumes the wrong one, the resumed terminal has `DYDO_AGENT=<wrongAgent>` baked into the resume body, and every command in that shell flows through the hijacked primitives. Issue [#0181](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md --> (saturate vs claim race) and [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md) (re-resume of already-resumed) are nearby concerns; not directly hijack-class but the resume body's DYDO_AGENT export is the same pivot point.
- **Severity:** **MEDIUM.** Indirect hijack vector — depends on the watchdog correctly identifying the agent, which other open issues ([#0181](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md)) note can race.

### Confirmed reproductions

#### R1. `dydo agent role` writes to Charlie when DYDO_AGENT=Charlie and the calling process owns Zelda

**Status:** Reproduced 3× in production (LC operator, 2026-05-18; see [issue #0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md -->). In-process test reproduction dispatched to test-writer Dexter (`identity-hijack-repro-role-set`) — **see Scout reports below for the test code and result**.
...
Reproduced by the LC operator ([issue #0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) timeline). The mechanism is: S4 creates a phantom file in agent A's inbox, attributed `from: <hijacked>`; A's UnreadMessages picks up the id (`MessageService.cs:88`); the guard's NOTICE (`GuardCommand.cs:991`) fires on every tool call for any process resolved to A; the operator's clear path (S1, S8) hijacks to a different agent and fails.
...
| S11 (worktree junction race) | Multi-worktree same agent name | Shared state.md write-write | [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) known surface | MEDIUM |
...
- **Issue:** [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) (already filed pre-inquisition — same root cause, no duplicate created)

#### F2. Round-2 hijack surface (`dydo agent role`) is unrelated to round-1 ([#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md)) and writes to other agents' state.md
...
- **Judge ruling:** CONFIRMED
- **Files examined:** `Commands/AgentLifecycleHandlers.cs` lines 172-213 (`ExecuteRole`); `Services/AgentRegistry.cs` lines 703-768 (`SetRole`); `Services/AgentSelector.cs` (post-[#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) review per [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) changelog); `dydo/project/changelog/2026/2026-05-18/fix-issue-0108-self-dispatch-hijack.md`.
- **Independent verification:** Confirmed the chain — `ExecuteRole` calls `registry.GetSessionContext()` (line 175) then `registry.GetCurrentAgent(sessionId)` (177) and `registry.SetRole(sessionId, …)` (184); `SetRole` at line 707 re-derives the agent via `GetCurrentAgent(sessionId)` then calls `UpdateAgentState(agent.Name, …)` at 737. The [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) fix is in `AgentSelector` (the dispatch target-selection codepath) and does not intersect any of these lines. F2 is a separate surface from F9.
- **Alternative explanations considered:** Could the symptom be the same [#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md) bug not yet rebuilt into the deployed binary at the LC site? Issue [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md)'s "Note on tool version" raises this. But the in-process repro (Dexter's failing test) runs against the current source on this branch, so it is decoupled from any deployment lag.
- **Issue:** [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) (already filed pre-inquisition — covers the same user-observable surface; no duplicate created)
...
- **Issue:** [#0183](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0183-identity-hijack-round-2-dydo-agent-role-mutates-other-agent-s-record-phantom-inb.md) (this is a systemic-scope statement about F1, not a separate bug; tracked alongside the root-cause issue)
...
- **Issue:** [#0189](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0189-tests-agentregistrytests-getsessioncontext-prefersdydoagentenvvar-overfile-and-g.md)
...
- **Issue:** [#0190](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0190-resolvesessionfallback-does-not-filter-by-assignedhuman-currenthuman-despite-the.md)
...
- **Issue:** [#0191](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0191-dydo-wait-stderr-suppressed-by-resume-bodies-in-all-three-terminal-launchers.md)
...
- **Issue:** [#0192](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0192-notice-handler-has-no-operator-escape-hatch-when-the-cited-inbox-file-is-unreach.md)
...
- **Issue:** [#0193](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0193-dydo-agent-claim-does-not-refuse-or-warn-when-dydo-agent-is-set-to-a-different-a.md)

#### F9. Round-1 fix ([#0108](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0108-dual-claim-of-same-agent-across-main-tree-and-worktree-breaks-guard-hook-in-orig.md)) addressed target selection, not sender identity
...
- **Issue:** [#0194](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0194-lifecycle-audit-events-record-the-hijacked-agent-because-agentregistry-setrole-r.md)
...
- **Issue:** [#0195](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0195-duplicate-wait-dos-an-attacker-with-dydo-agent-x-can-hold-x-s-general-wait-slot.md)
...
- **Issue:** [#0196](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0196-two-phase-storesessioncontext-in-handledydobashcommand-publishes-an-unverifiable.md)
...
- **Issue:** [#0197](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0197-watchdog-and-terminal-launchers-do-not-scrub-or-pin-dydo-agent-on-child-processs.md)
...
- **HP3:** Watchdog auto-resume mis-identifies and resumes the wrong agent, baking DYDO_AGENT=wrong into the resumed shell. Not reproduced in this inquisition — depends on [#0181](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0181-saturate-vs-claim-race-watchdog-overwrites-freshly-claimed-agent-attempts-cap.md) / [#0143](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0143-watchdog-re-resumes-already-resumed-agent-on-subsequent-ticks-3-terminals-for-th.md) racing, which are open but separate. **Status:** plausible indirect hijack vector; flagged as adjacent.
