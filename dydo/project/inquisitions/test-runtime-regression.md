...
# Inquisition: Test Runtime Regression ([#0148](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md) follow-up + [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) cross-reference)

Re-prosecution of the test-suite runtime story prompted by balazs's observation that the prior `Git()`-helper fix (commit `aeee461`, "drain stdout/stderr concurrently in WorktreeMergeSafety Git helper") restored the **steady-state** baseline but did not address the **underlying mechanism** the original investigation hinted at. The downstream symptom in [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) (xunit Console-capture cross-over under coverage) is consistent with the same broken isolation still being live. Investigation only — no code/test changes.

## 2026-05-05 — Charlie

### Scope

- **Entry point:** Feature investigation, dispatched by Adele on behalf of balazs to re-open [#0148](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md) in light of [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md).
...
- **Files investigated (test sites):**
  ...
  - `DynaDocs.Tests/Services/AgentRegistryTests.cs:2970-3016` ([#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) failure #1)
  - `DynaDocs.Tests/Commands/WorktreeMergeSafetyIntegrationTests.cs:1-296`
  - `DynaDocs.Tests/Commands/WorktreeCommandTests.cs:516-546, 2905-2955` ([#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) failure #2)
...
1. **Cross-collection static-state mutation.** xUnit collection definitions are misconfigured: only `Integration` actually disables parallelization between collections. `ProcessUtils`, `ConsoleOutput`, and the implicit "no-collection" classes can run in parallel, yet they all mutate the same process-global statics (`Console.Out/Error`, `ProcessUtils.IsProcessRunningOverride`). [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s Console cross-over is the most visible face of this; the live `QueueServiceTests` flake observed during this investigation is another. The "worktree-lock fix" balazs remembered was real (`Services/WorktreeCreationLockTests.cs`, lock-file serialisation in `DispatchService.CreateGitWorktree`) but it solves a different problem (concurrent agent dispatches stomping on each other's worktree dirs); it has no bearing on in-process xunit isolation.
...
- **Classification:** root-cause (of [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) cross-over and the QueueServiceTests live flake; contributing-factor for runtime variance via retried-flake tests).
- **Severity:** high.
...
- **Evidence:**
  ...
  - **[#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s smoking-gun cross-over is exactly this pattern:**
...
- **Proposed fix path:**
  ...
  - Long-term cleaner alternative (per [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) path 1): migrate Console-asserting tests to xunit's `ITestOutputHelper`, and have production code accept a redirectable writer. Larger blast radius — call out as a separate decision rather than bundling into the fix batch.
...
- **Independent verification:**
  ...
  - **Caveat on the `AgentRegistryTests` connection:** Charlie ties [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s failure #1 (`IncrementResumeAttempts_ConcurrentCalls_ProduceExactCount` `Expected: 10, Actual: 9`) to this finding via "AgentRegistryTests has no [Collection], so it parallelises with ProcessUtils and ConsoleOutput". That linkage is weak — `IncrementResumeAttempts` does not touch `Console.Out/Error` or `IsProcessRunningOverride`. The race is internal to its file-write/read cycle (`File.Move` racing with AV/indexers under heavy parallel load, per the test's own comment at lines 2989-2992). Adding `DisableParallelization` won't directly fix that bug; it may *mitigate* it by reducing concurrent disk pressure, but [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s failure #1 likely needs its own probe (as [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) itself flagged: "warrants its own probe before being chalked up to test timing under coverage"). Issue [#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md)<!-- manifest duplicate: https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md --> explicitly does not claim to fix it.
...
- **Issue:** [#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md)
...
- **Issue:** [#0168](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0168-git-helper-redirect-without-drain-pattern-in-snapshotservicetests-rungit-and-inq.md)
...
- **Issue:** [#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md)
...
- **Classification:** root-cause (of [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s "exit 0 with three failures" trap) and contributing-factor for runtime perception (the "real" runtime is hidden because re-runs are needed to see failures).
...
- **Evidence:**
  ...
  - `DynaDocs.Tests/coverage/gap_check.py:716-719`:
    ...
    Exit code is conditioned only on coverage-tier failures and registry errors — not on `tests_ok`. So a run where `dotnet test` returns non-zero (e.g. one `[FAIL]` on a flake) but every module still meets its tier exits 0. This is exactly the trap [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) documented and the reason the embedded failures are silent to the gate.
...
- **Alternative explanations considered:** could the divergence be intentional ("the gate's job is coverage, not tests")? No — both gates ultimately ask "is the test suite green?" and the docstring at `gap_check.py:1-25` and the issue Resolution sections in [#0148](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md) and [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) both treat `gap_check`'s exit code as authoritative. Charlie's reading is correct.
- **Issue:** [#0169](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0169-gap-check-py-does-not-propagate-dotnet-test-exit-code-gate-signal-lies-about-tes.md)
...
1. **[#0169](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0169-gap-check-py-does-not-propagate-dotnet-test-exit-code-gate-signal-lies-about-tes.md)** (`gap_check.py` exit propagation) first — without it, every regression test for [#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md) / [#0168](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0168-git-helper-redirect-without-drain-pattern-in-snapshotservicetests-rungit-and-inq.md) / [#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md) is silent, which means we can't reliably verify the other fixes.
2. **[#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md)** (collection isolation, two-step) second — landing 1a (gate-bypass migration) and 1b (DisableParallelization) together is the right call, for the dependency reasons Charlie articulates.
3. **[#0168](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0168-git-helper-redirect-without-drain-pattern-in-snapshotservicetests-rungit-and-inq.md)** (test-side git-helper drain) any time after [#0169](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0169-gap-check-py-does-not-propagate-dotnet-test-exit-code-gate-signal-lies-about-tes.md) lands — mechanical, low risk, independent of [#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md).
4. **[#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md)** (production redirect-without-drain) tracked separately — different blast radius, different testing burden, not bundled.
...
- **[#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md)** (was #NEW-A): Test parallelism breaks process-global static isolation — Finding #1. Severity: high. Area: general.
- **[#0168](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0168-git-helper-redirect-without-drain-pattern-in-snapshotservicetests-rungit-and-inq.md)** (was #NEW-B): Git-helper redirect-without-drain in `SnapshotServiceTests.RunGit` and `InquisitionTests.InitGitRepo` — Finding #2. Severity: medium. Area: backend.
- **[#0169](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0169-gap-check-py-does-not-propagate-dotnet-test-exit-code-gate-signal-lies-about-tes.md)** (was #NEW-C): `gap_check.py` does not propagate `dotnet test` exit code — Finding #4. Severity: high. Area: general.
- **[#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md)** (was #NEW-D): Production `Process.Start` callers redirect both streams but read only stdout — Finding #3. Severity: medium. Area: backend.
- **(Resolution-update suggestion for [#0148](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md)):** the original fix is correct and stays — but the issue's "Other lower-priority findings from this investigation" bullet under Related is incomplete. Add pointers to [#0167](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0167-test-parallelism-breaks-process-global-static-isolation-collection-definitions-m.md) / [#0168](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0168-git-helper-redirect-without-drain-pattern-in-snapshotservicetests-rungit-and-inq.md) / [#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md) and (if [#0148](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0148-test-suite-runtime-ballooned-from-3min-to-10min-investigate-parallelism-other-sp.md) is being closed) re-frame as "narrow fix; broader pattern tracked under #0167–[#0170](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0170-production-process-start-callers-redirect-both-streams-but-read-only-stdout-same.md)".
...
- Whether the in-[#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md) cross-over reproduces at this commit. It didn't fire on this run, but [#0165](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/0165-tests-under-coverage-produce-3-unrelated-failures-xunit-console-capture-isolatio.md)'s reporter (also Charlie, on a different branch) saw it on `gap_check` runs on this same day. The race is real per Finding #1's code analysis; reproducing it on demand requires running gap_check enough times to land the timing window — out of scope for this inquisition.
