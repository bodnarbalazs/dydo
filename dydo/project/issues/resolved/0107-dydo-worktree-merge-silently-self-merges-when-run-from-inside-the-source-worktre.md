---
title: `dydo worktree merge` silently self-merges when run from inside the source worktree, then consumes markers
area: general
fix-release: 
needs-human: false
resolution: 
...
## Resolution
Fixed in commit 9cd0fc7 (Iris) + tests in commit e587947, merged via 3ba24c4. Approach: rehome to main repo via .worktree-root marker + ancestor-gated marker preservation in FinalizeMerge + consolidated finalize output (no more 'Merge finalized' alongside 'cannot delete branch ... used by worktree'). 7 regression tests covering self-merge guard, ancestor-check failure, branch-delete failure, true-no-op-vs-self-merge, conflict, rehome from inside worktree, and defensive worktree-root check. 3788/3788 pass, gap_check 100%. Reviewed by Charlie (review file at dydo/agents/Charlie/review-fix-worktree-merge-self-merge-redo.md). Merge code-writer: Adele. Docs guide updated in commit 756f71e (Charlie). NOTE: this was a redo — the first attempt (Grace) shipped uncommitted and was destroyed by worktree cleanup on release; lessons-learned filed as #0110 (no requires-commit gate) and #0108 (dual-claim across worktrees).