---
title: Reviewer cannot dispatch merge code-writer when dispatch chain originates from a released test-writer (or other ineligible role)
area: backend
fix-release: 
needs-human: false
resolution: 
...
## Resolution
Outdated: reviewer.role.json dispatch-restriction now accepts test-writer as dispatcher (requiredRoles [code-writer, test-writer]) and the --queue merge mechanism the issue centers on was removed in the 2.0 pivot. Triage sweep 2026-07-04 (Brian, CoS).