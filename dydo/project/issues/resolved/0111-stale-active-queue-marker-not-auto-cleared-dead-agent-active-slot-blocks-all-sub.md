---
title: Stale active-queue marker not auto-cleared; dead-agent active slot blocks all subsequent --queue merge dispatches indefinitely
area: backend
fix-release: 
needs-human: false
resolution: 
...
## Resolution
Outdated by the 2.0 pivot: the merge-queue active-slot subsystem was removed — no --queue dispatch, no QueueService, no _active.json lifecycle at HEAD (only a leftover ConfigFactory.DefaultQueues string, which the legacy-sweep inquisition should catch). Triage sweep 2026-07-04 (Brian, CoS).