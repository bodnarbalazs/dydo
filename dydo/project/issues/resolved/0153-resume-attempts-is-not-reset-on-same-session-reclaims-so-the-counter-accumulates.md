---
title: resume-attempts is not reset on same-session reclaims, so the counter accumulates across crash episodes (decision 022 mismatch)
area: backend
fix-release: 
needs-human: false
resolution: 
...
## Resolution
Fixed at HEAD: same-session reclaim resets ResumeAttempts=0 and LastResumeLaunchedAt=null (AgentRegistry.cs:467-473, comment cites #0153). Goes live with the 2.0 install. Triage sweep 2026-07-04 (Brian, CoS).