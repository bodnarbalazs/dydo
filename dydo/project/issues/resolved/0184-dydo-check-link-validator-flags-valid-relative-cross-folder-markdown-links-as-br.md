...
Fixed at HEAD: root-caused by #0185; CheckDocValidator now scans allDocs from the docs root and passes the full set to every rule, so valid cross-folder links resolve (CheckDocValidator.cs:20-54; regression tests in BrokenLinksRuleTests). Triage sweep 2026-07-04 (Brian, CoS).