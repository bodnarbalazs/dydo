...
Fixed at HEAD with fix shape 1: Validate scans allDocs from the docs root and treats the subfolder as a reportScope filter (CheckDocValidator.cs:13-56). Unit-test gap tracked separately as #0206 (still open). Triage sweep 2026-07-04 (Brian, CoS).