...
Outdated by the 2.0 pivot: the oversight-role --no-wait confirm-intent gate no longer exists in the dispatch path (judge oversight role removed), so the exit-2-mutates-before-refusal flow is gone; only the 'already working on task' rejection remains (DispatchService.cs:53). Triage sweep 2026-07-04 (Brian, CoS).