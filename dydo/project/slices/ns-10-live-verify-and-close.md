...
7. Verify two remaining live assumptions the fake cannot settle:
   - **Batch-append an existing table past 100 rows** — ~~`NotionBlockAppender.GuardTableWidth` throws for a table wider than 100 rows~~ **CONFIRMED & IMPLEMENTED 2026-07-22 (issue 0299 F19):** live probe proved that appending `table_row` children to an EXISTING table via `PATCH /blocks/{table_id}/children` WORKS. The `GuardTableWidth` loud-abort is lifted; `NotionBlockAppender.Cut` now row-batches — a table's first 100 rows ride inline in the create/append, the remainder defer to the returned table block id and append in ≤100 chunks. Pinned by `NotionBlockAppenderTests.WideTable_*` and the retargeted `NotionLiveWideTableTests`.
