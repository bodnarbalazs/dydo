...
4. Throttling: rely on `NotionClient`'s built-in 3 req/s throttle; no additional pacing needed beyond the interval floor.
5. Update `dydo/reference/dydo-commands.md` + `Templates/dydo-commands.template.md` (watchdog section), `dydo/understand/architecture.md` (Watchdog section — currently says "a stub"), and close `dydo/project/tasks/notion-sync-daemon.md` **recording the descopes explicitly in the task file**: the guard-trigger self-start from the original scope is deliberately dropped (a manually started daemon is the v1), and the ~15s interval became a 60s default with a 15s floor. Descoped items that still matter move to a backlog note, not silence.
