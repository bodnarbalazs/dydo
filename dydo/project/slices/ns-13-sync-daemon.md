...
4. **Cheap ticks are CORE SCOPE, not optional** (supersedes the earlier 'consider if cheap' note): 99%+ of ticks find nothing changed and must not read or download record bodies to discover that.
   ...
   - **Deletion semantics on fast ticks (explicit design decision):** a filtered query cannot see archived pages, so fast ticks do NOT detect remote deletions. A FULL sweep (today's complete reconcile, fuse-guarded as always) runs every Nth tick (~every 10 minutes at 15s ticks) and on every manual `dydo notion sync` — remote deletions are rare and tolerate minutes of latency; local deletions ARE detected every tick (the mtime sweep sees the file gone).
