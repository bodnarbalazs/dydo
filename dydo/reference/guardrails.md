...
| H20 | `dydo wait` in foreground | `dydo wait` executed without `run_in_background: true`. Would block the agent's main thread. Exempts the immediate-return forms `dydo wait --cancel` and `dydo wait --register` (c1-2, #0254 — the durable-wait registration form codex hosts must use, since a codex hook input carries no `run_in_background` field). | `BLOCKED: 'dydo wait' must run in background.` |
