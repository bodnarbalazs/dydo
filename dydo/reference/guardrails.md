...
| S6 | Agent tool soft-nudge | Agent uses Claude Code's built-in `Agent` tool (which spawns a stateless sub-agent outside dydo's identity/role/audit chain). Steers toward `dydo dispatch` for delegation to a stateful dydo agent. | `.agent-tool-nudge` | `Claude Code's 'Agent' tool spawns a stateless subagent that bypasses dydo's identity, role, and audit guarantees. Prefer 'dydo dispatch'... If you have a clear reason to use the built-in Agent tool... run the call again and it will pass.` |
