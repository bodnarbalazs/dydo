...
All spine state under the gitignored `dydo/_system/.local/` tree is **parent-page-scoped** ([issue 0257](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0257-notion-reset-parent-page-archives-the-real-board-s-databases-provision-state-is.md)), keyed by `ParentPageKey.Hash8(parentPageId)` — the same helper `DocsTreeSync.SnapshotAdapterName` uses for the docs mirror — so a scratch `--parent-page` target's state is disjoint from the configured board's and a `notion reset --parent-page <scratch>` can never archive or poison the real board. `NotionSpineState` is the one place that resolves these paths (for both `sync` and `reset`). The parent id is canonicalized first (`ParentPageKey.Normalize`: lowercase, strip dashes), so the dashed and undashed forms of the same page id resolve to one state and an override equal to the configured page — in either form — counts as non-override.
...
The suite covers, one focused test each: spine provisioning of all seven types with formulas accepted; the [0290](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0290-notion-sync-spine-database-rows-created-with-no-title-every-card-shows-new-page.md) title fallback; the [0291](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0291-notion-sync-page-create-with-body-does-not-chunk-children-100-400-on-large-bodie.md) >100-block create-then-append (whose body also carries a >2000-char run, covering constraint 3); a 3-deep nested list (ns-6); every code-fence language alias; the [0278](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0278-notion-futurefeature-board-page-titles-render-as-new-page-record-title-not-writt.md) FutureFeature title + status options; [0257](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0257-notion-reset-parent-page-archives-the-real-board-s-databases-provision-state-is.md) reset scoping (scratch reset leaves a second parent's state file untouched); the ns-5 recovery DTO wire shapes (`SearchDataSources` computed name + `parent.database_id`, `ListViews` id-only bare refs — names come from `RetrieveView(id)`, `RetrieveDatabase` parent); a table + quote + `[!missing]` body round-trip; and a >100-row table landing via row-batching ([issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md) F19).
...
- **`gate-result` was a `select`** whose values are free-prose verdict sentences containing commas → `Invalid select option, commas not allowed`. Retyped to `rich_text`; the `health` and `attention` formulas match it case-insensitively with `test(prop("gate-result"), "(?i)fail")` ([issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md) F4 — the formula dialect supports `test()` regex; `lower()` does NOT exist, live-probed). Known limitation: `test()` still substring-matches "fail" inside passing prose like "no failures", flagging a false Off Track — accepted as the safe direction for a health flag.
- **Field/body equilibrium (four asymmetry classes, [issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md))**: a full reset+push then `dydo notion sync --dry-run` initially planned rewrites for 196 of 397 records; driven to **zero** by canonicalizing absent==empty==default (checkbox/scalar schema-default echoes), recognizing the synthesized-title echo, preserving the local body on field-only writes, and comparing fields order-insensitively (frontmatter order vs. Notion's canonical echo order). See the ns-10-core commit and [issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md).
- **Table row-batching** ([issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md) F19, live-confirmed 2026-07-22): appending `table_row` children to an EXISTING table via `PATCH /blocks/{table_id}/children` works, so a table wider than the 100-row per-request cap now creates with its first 100 rows inline and appends the remainder to the returned table block id in ≤100 chunks — the old `GuardTableWidth` loud-abort is lifted.
- **Property clears** ([issue 0299](https://github.com/bodnarbalazs/dydo/blob/ffffc02dcdf92b9677d0eb4f522d1af57a869990/dydo/project/issues/resolved/0299-ns-10-live-equilibrium-four-field-body-asymmetry-classes.md) F5, live-probed): a local clear of a `select`/`date`/`number`/`url` pushes the explicit clear shape (`{"select": null}`, `{"date": null}`, …; `{"rich_text": []}` for rich_text) on an update, so a blanked field is removed on the board instead of silently reverting.
...
### Auto-start + activity lease (watchdog-autostart-lease)

The daemon no longer has to be started by hand. The guard hook — which already runs on every tool call — refreshes an **activity stamp** (`_system/.local/watchdog-activity`) at most once every 5 minutes and, when the stamp is stale, attempts an auto-start. Auto-start and lease expiry are complements: neither is safe alone.

- **Guard stays fast.** The per-call cost is a single stat of the stamp: a stamp younger than 5 minutes returns immediately, no process probe. Only a stale/missing stamp pays the refresh + start attempt, and the whole trigger is wrapped so it can never break or block a tool call.
- **Auto-start preconditions** (all must hold, else it silently skips): no suppress marker (below); Notion actually configured (the same `DaemonConfigError` gate the manual `start` uses — token + project + parent page); prior provision evidence in this project (a `provision*.json` under `_system/.local/notion/`, written only after a successful provision/sync); and no live daemon already holding the pid. The provision-evidence gate means an implicit trigger only ever *resumes* a sync explicitly begun in this project — a never-synced project is never auto-started, even when a global `DYDO_NOTION_PARENT_PAGE` + token would otherwise satisfy the config gate. A real spawn is quiet — no stdout — beyond one `NOTICE: dydo watchdog auto-started (Notion sync)` line on stderr.
- **Lease expiry.** Each tick, after syncing, the daemon compares now against `max(stamp last-write, its own process-start time)`. Once that exceeds the lease (default **60 min**, `--lease <minutes>`, `--lease 0` disables), it logs `lease_expired` and exits cleanly. The process-start baseline means a freshly started daemon is never insta-killed by a missing or stale stamp. Net: the daemon runs while someone works in the project, dies about an hour after work stops, and resurrects on the next session's first tool call.
- **Stop means stop.** `dydo watchdog stop` writes a suppress marker (`_system/.local/watchdog.hold`) *before* killing, and the marker is honored in two places: auto-start skips while it exists, and the daemon itself checks for it every tick, logging `hold_honored` and exiting cleanly when found. The per-tick check is the load-bearing guarantee — it closes both the stop-vs-autostart race (a daemon spawned in the stop window notices the marker after at most one tick) and the failed-kill case (a daemon that survived `stop` still honors the hold). The marker persists until the next manual `dydo watchdog start` clears it — an explicit stop is never silently undone by the next tool call. A crashed daemon leaves no marker, so crashes still self-heal. Both a manual `start` and an auto-start touch the activity stamp at spawn time, so a fresh daemon always begins with a full lease.
